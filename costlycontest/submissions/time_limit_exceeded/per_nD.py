#!/usr/bin/env python3
# time complexity Omega(n*D) where D = Omega(m*d) is number of
# possible problem set difficulties possible.
import bisect
import heapq
...
poss_diffs = {0}
for diff in diffs:
    poss_diffs |= set(x + diff for x in poss_diffs)
skill_separators = list(sorted(t // diff for diff in poss_diffs if diff))
