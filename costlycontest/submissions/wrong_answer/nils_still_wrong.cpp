...
int main() {
    ...
    int ans = 0;
    if(n_fast > 0)ans = slows;
    rep(c1,0,k){
        if(slowness[k-c1-1] >= too_slow)slows++;
        if(slowness[k-c1-1] < too_slow)n_fast++;

        if(slowness[n-c1-1] >= too_slow)slows--;
        if(slowness[n-c1-1] < too_slow)n_fast--;
        if(n_fast > 0 && slows > ans)ans = slows;
    }
    cout << n - ans << "\n";
